...
def fetch_results(offset, limit, search_type, essence_title):
    if search_type == "Like":
        response = requests.get(f"http://localhost:8000/essence/like?essence_title={essence_title}&limit={limit}&offset={offset}")
    else:
        response = requests.get(f"http://localhost:8000/essence/exact?essence_title={essence_title}&limit={limit}&offset={offset}")
...
if st.sidebar.button("Search"):
    st.session_state['offset'] = 0
    results = fetch_results(st.session_state['offset'], PAGE_SIZE, search_type, essence_title)
